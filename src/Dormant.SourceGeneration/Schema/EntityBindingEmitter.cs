...
internal static class EntityBindingEmitter
{
    ...
    public static string Emit(string @namespace, EntityModel entity, NamingConvention convention)
    {
        ...
        var tableSql = "\"" + Table(entity, convention) + "\"";
        var columnsSql = string.Join(", ", columns.Select(p => "\"" + Col(p, convention) + "\""));
        var placeholders = string.Join(
            ", ",
            columns.Select((_, i) => "$" + (i + 1).ToString(CultureInfo.InvariantCulture)));
        var insertSql = $"INSERT INTO {tableSql} ({columnsSql}) VALUES ({placeholders})";
        ...
        EmitSelectByKey(writer, entity, columnsSql, tableSql, key, convention);
        EmitSnapshot(writer, entity, columns);
        EmitUpdate(writer, entity, key, token, setCols, convention);
        EmitDelete(writer, entity, tableSql, key, token, convention);
        ...
        NamingConvention convention)
    {
        ...
        {
            var selectSql = $"SELECT {columnsSql} FROM {tableSql} WHERE \"{Col(key, convention)}\" = $1";
            writer
                .Line($"return new {Abs}.Querying.PreparedStatement(")
        ...
        NamingConvention convention)
    {
        ...
        writer
            .Line($"var sql = new global::System.Text.StringBuilder(\"UPDATE \\\"{Table(entity, convention)}\\\" SET \");")
            .Line("int p = 0;")
        ...
        NamingConvention convention)
    {
        ...
        if (token is null)
        {
            var deleteSql = $"DELETE FROM {tableSql} WHERE \"{Col(key, convention)}\" = $1";
            writer.Line($"return new {Stmt}({Quote(deleteSql)}, writer => writer.Write(1, e.{keyName}));");
        }
        else
        {
            var tokenName = Naming.ToPascalCase(token.Name);
            var deleteSql = $"DELETE FROM {tableSql} WHERE \"{Col(key, convention)}\" = $1 AND \"{Col(token, convention)}\" = $2";
            writer
