...
internal static class EntityBindingEmitter
{
    ...
    public static string Emit(string @namespace, EntityModel entity, NamingConvention convention)
    {
        ...
        // runtime by generated code (changed-columns-only), so it stays string-based by nature.
        var table = Table(entity, convention);
        var colNames = columns.Select(p => Col(p, convention)).ToList();
        var insertSql = SqlRenderer.Render(new InsertStatement(table, colNames));
        ...
        EmitSelectByKey(writer, entity, table, colNames, key, convention);
        EmitSnapshot(writer, entity, columns);
        EmitUpdate(writer, entity, key, token, setCols, convention);
        EmitDelete(writer, entity, table, key, token, convention);
