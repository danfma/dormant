...
public sealed class ConnectivityTests
{
    ...
    public async Task No_boxing_io_round_trips_against_real_postgres()
    {
        ...
        await using var dataSource = DormantPostgres.CreateDataSource(postgres.GetConnectionString());

        await using (var session = await dataSource.OpenAsync())
        {
            await session.BeginAsync();
            await session.ExecuteAsync(new PreparedStatement(
                "CREATE TABLE t (n integer not null, label text not null)"));
            await session.ExecuteAsync(new PreparedStatement(
